...
${context.latestReport ? `Wellbeing Score: ${context.latestReport.overallScore}/100
Strengths: ${context.latestReport.psychologicalTraits?.filter(t => t.score >= 70).map(t => t.trait).join(', ')}
Areas to support: ${context.latestReport.areasOfConcern?.join(', ')}` : 'No assessment data available'}
...
Profile:
- Wellbeing Score: ${context.latestReport.overallScore}/100
- Psychological Traits: ${context.latestReport.psychologicalTraits?.map(t => `${t.trait}: ${t.score}/100`).join(', ')}
...
- Resilience: ${context.latestReport.psychologicalTraits?.find(t => t.trait === 'Resilience')?.score || 'N/A'}/100
- Stress Management: ${context.latestReport.psychologicalTraits?.find(t => t.trait === 'Stress Management')?.score || 'N/A'}/100
- Emotional Regulation: ${context.latestReport.psychologicalTraits?.find(t => t.trait === 'Emotional Regulation')?.score || 'N/A'}/100` : ''}
...
Current Status:
- Wellbeing Score: ${context.latestReport?.overallScore || 'N/A'}/100
- Recent Activity: ${context.recentJournals.length} journals, ${context.meditationHistory.length} meditations
${historicalReports && historicalReports.length > 1 ? `- Historical Scores: ${historicalReports.map(r => r.overall_score).join(' → ')}` : ''}
    ...
    res.json({
      ...
      data: {
        ...
        stats: {
          ...
          currentScore: context.latestReport?.overallScore || null,
